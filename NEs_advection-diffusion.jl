...
# ╔═╡ 32455e04-efd0-11f0-ac67-ddb515df7f1b
begin
	using LinearAlgebra
	using SparseArrays
...
# ╔═╡ ebf59748-84cd-4682-a5e7-0b1c71e43c19
md"""
$∂_t u - \frac{1}{\text{Pe}} \Delta u + \mathbf{a} \cdot ∇ u = 0 \text{ in } Ω = (-1,1) \times (0,1),$
$u = 1 - \text{tanh(Pe) on } Γ_0,$
$u = 1 + \text{tanh}((2x+1)\text{Pe}) \text{ on } Γ_\text{in},$
$\frac{∂u}{∂n} = 0 \text{ on } Γ_\text{out},$
$u(x,0) = u_0 (x) \text{ in } Ω$

The convective field is given as 
$\mathbf{a}(x,y) = \begin{bmatrix}
	2y(1-x^2) \\\
	-2x(1-y^2)
\end{bmatrix}, \;\; (x,y) \in Ω$
$Γ_\text{in} := [-1,0] × \{0\}, Γ_\text{out} = [0,1] × \{0\},\text{Pe} = 10$
"""


# ╔═╡ f2cef702-ee9d-4204-bc80-1efc59e6ed5f
grid = togrid("ad_msh_2.msh")

# ╔═╡ 974c82a3-230a-404f-a4b5-ab0161f0e76e
begin
	Pe = 10
	order = 1
	...
	cellvals = CellValues(qr,ip)
end

# ╔═╡ 887c58b4-ebae-4ddf-b4f4-6c4c2016fe4c
begin
	dh = DofHandler(grid)
	add!(dh, :u, ip)
	close!(dh)
...
# ╔═╡ b34a4567-5c6c-47db-a900-2d3dc11f986d
function velocity(x)
	return SVector(
		2*x[2]*(1-x[1]^2),
		-2*x[1]*(1-x[2]^2)
...
# ╔═╡ 611596ba-a69d-4444-8d9d-cdc6f1d1089d
begin
	function inflow_bc(x,Pe)
		1 + tanh((2*x[1] + 1) * Pe)
	end
	
	function wall_bc(x,Pe)
		1 - tanh(Pe)
	end

	ch = ConstraintHandler(dh)
	
	drbc1 = Dirichlet(:u, getfacetset(dh.grid, "Inflow"),
				 	(x,t) -> inflow_bc(x,Pe)
				 )
	drbc2 =	Dirichlet(:u, getfacetset(dh.grid, "Wall"),
				 	(x,t) -> wall_bc(x,Pe)	
				 )
	add!(ch, drbc1)
	add!(ch, drbc2)
...
# ╔═╡ e6e8d8a8-3dd3-405e-b445-1e78d9c2afb4
begin
	n = ndofs(dh)

	A_diff = allocate_matrix(dh)
	A_adv = allocate_matrix(dh)
	M_lump = zeros(n)
	
end

# ╔═╡ b9c45f6b-722d-463e-912d-a894efae9777
begin
	assembler_diff = start_assemble(A_diff)
	assembler_adv = start_assemble(A_adv)

	nb = getnbasefunctions(cellvals)
	
	for cell in CellIterator(dh)
		reinit!(cellvals, cell)

		Ke = zeros(nb,nb)
		Ae = zeros(nb,nb)
		Me = zeros(nb,nb)

		for q ∈ 1:getnquadpoints(cellvals)
			dΩ = getdetJdV(cellvals,q)
			∇ϕ = [shape_gradient(cellvals, q, i) for i ∈ 1:nb]
			ϕ = [shape_value(cellvals, q, i) for i ∈ 1:nb]

			xq = spatial_coordinate(cellvals, q, getcoordinates(cell))
			a = velocity(xq)


			for i ∈ 1:nb, j ∈ 1:nb
				Ke[i,j] += (1/Pe) * dΩ * dot(∇ϕ[i], ∇ϕ[j])
			end
		...
		assemble!(assembler_diff, celldofs(cell), Ke)
		assemble!(assembler_adv, celldofs(cell), Ae)
...
# constant g term from inhomogeneous Dirichlet B.C's
begin 
	g = zeros(size(A)[1])
	apply!(g, ch)
...
# Apply boundary conditions to discretization
apply!(A,ch)

# ╔═╡ 5074073b-61fa-4b0d-9595-b4d718de700b
# Multiply by sqrt lumped mass
begin
	u_0 = zeros(size(A)[1])
	sqrtM = sqrt.(M_lump)
	isqrtM = 1 ./ sqrtM
	...
	Lh = -Diagonal(isqrtM) * A * Diagonal(isqrtM)
...
begin
	p = Params(90,200,10e-16,false,exact_input,0.95)
...
function benchmark_function(funm_krylov, f, A, b, Ainvg, ps, exact)
	...
	for p ∈ ps
		...
		final_res = res[1] - Ainvg
		
		err = norm(exact - final_res)
		...
		push!(codes, message(res[2]))
	end
	return times,allocs,errs,mvps,restarts,codes
end

# ╔═╡ 8f62ee16-3d29-4a97-9e41-99a753de50e0
ps = [Params(m,40,1e-16,true,nothing,0.95) for m ∈ 30:10:200]

# ╔═╡ 840783f0-6424-4ba0-9184-218547364c42
ra_times,ra_allocs,ra_errs,ra_mvps,ra_restarts,ra_codes = benchmark_function(funm_krylov_restart_ra, r, tLh, b, Ainvg, ps, exact_u)

# ╔═╡ e7514d9d-b500-4e2b-9ac8-9330032032b3
full_times,full_allocs,full_errs,full_mvps,full_restarts,full_codes = benchmark_function(funm_krylov_restart_full, exp, tLh, b, Ainvg, ps, exact_u)
...
function plot_times(df1,df2,savefig=true)
	...
	df1_noconv = subset(df1, :converged => c -> .!c)
	df2_noconv = subset(df2, :converged => c -> .!c)

	xsymb = :Restart_Length
	xname = "Restart Length"
	ysymb = :Times
	yname = "Time (s)"
	...
	plt1 = plot_formula(df1_converged, xsymb, xname, ysymb, yname, ra_c, :solid, :utriangle, "R.A. converged")
	plt2 = plot_formula(df1_noconv, xsymb, xname, ysymb, yname, ra_c, :dash, :xcross, "R.A. no conv.")
	plt3 = plot_formula(df2_converged, xsymb, xname, ysymb, yname, full_c, :solid, :dtriangle, "Full converged")
	plt4 = plot_formula(df2_noconv, xsymb, xname, ysymb, yname, full_c, :dash, :cross, "Full no conv.")
...
function plot_errs(df1,df2,savefig=false)
	...
	d = draw(plt, axis = (; yminorticksvisible = true, yminorgridvisible = true,
        yminorticks = IntervalsBetween(5), yscale = log10),
		 legend =(
			 position=:bottom, 
			 framevisible=false,
			),
...
function build_df(ps,rs)
	...
	mvps_bound = [k * ps[2].restart_length for k ∈ 1:length(rs[2].upbnd)]
	
	dfexact = DataFrame([rs[1].errs,mvps_exact],[:Errors,:MVPs])
	dfupbnd = DataFrame([rs[2].upbnd,mvps_bound],[:Errors,:MVPs])
	dflowbnd = DataFrame([rs[2].lowbnd,mvps_bound],[:Errors,:MVPs])
...
function build_single_plot(dfs::Tuple{DataFrame,DataFrame,DataFrame},c1,c2,m1,m2,l1,l2)
	...
	plt3 = data(dfs[3]) * mapping(:MVPs,:Errors) * visual(Lines, linestyle=:dash) * visual(color = c2, label = l2) 
	plt = plt2 + plt3 + plt1
	return plt
end
...
function test_and_plot(r, f, A, b, restart_length, exact)
	...
	p_exact = Params(restart_length,100,stop_acc,false,exact,0.95)
	p_bound = Params(restart_length,100,stop_acc,true,nothing,0.95)

	out_exact_ra = funm_krylov_restart_ra(r,A,b,p_exact)
	out_bound_ra =  funm_krylov_restart_ra(r,A,b,p_bound)
	
	out_exact_full = funm_krylov_restart_full(f,A,b,p_exact)
	out_bound_full =  funm_krylov_restart_full(f,A,b,p_bound)

	
	
	plt1 = build_single_plot(
		build_df(
			(p_exact,p_bound),
			(out_exact_ra[3],out_bound_ra[3])
		),
		:cornflowerblue,:tomato,:utriangle,:x,
		"RA Abs. Error",
		"RA Error Indictors" 
	)
	plt2 = build_single_plot(
		build_df(
			(p_exact,p_bound),
			(out_exact_full[3],out_bound_full[3])
		),
		:seagreen4,:red3,:dtriangle,:cross,
		"Full Abs. Error",
		"Full Error Indicators"
	)
	plt = plt1 + plt2
	draw(plt, 
		 axis = (; 
		 ...
		 figure = (;
			 title = "Advection-Diffusion Equation Error for Restart Length $(restart_length)",
			 size=(650,650)
		 )
		)
