...
md"""
$∂_t u - \Delta u = 0 \text{ in } \Omega = (0,1)^3, t > 0$
$u(x,t) = 0 \text{ on } Γ = ∂Ω, t > 0$
$u(x,0) = u_0(x) \text{ in } Ω$
		
Discretized by the usual seven-point stencil on a uniform grid involving $$n_1$$ interior grid points in each Cartesian direction. 
"""
...
function plot_times(df1,df2,savefig=true)
	...
	plt2 = data(df2) * mapping(:Restart_Length => "Restart Length",:Times => "Time (s)") * (smooth() + visual(Scatter, marker=:+)) * visual(color = :tomato, label = "Full") 
...
function plot_errs(df1,df2,savefig=false)
	...
	plt2 = data(df2) * mapping(:Restart_Length => "Restart Length",:Absolute_Errors => "Abs. Error") * (visual(Scatter, marker=:+)) * visual(color = :tomato, label = "Full") 
...
function plot_allocations(df1,df2,savefig=false)
	...
	plt2 = data(df2) * mapping(:Restart_Length => "Restart Length",:Allocations => "Allocations (mb)") * (visual(Scatter, marker=:+)) * visual(color = :tomato, label = "Full") 
	plt = plt1 + plt2
	d = draw(plt,
		 legend =(
			 position=:top, 
...
function plot_restarts(df1,df2,savefig=false)
	...
	plt2 = data(df2) * mapping(:Restart_Length => "Restart Length",:Restarts => "Restarts Needed") * (visual(Scatter, marker=:+)) * visual(color = :tomato, label = "Full") 
	plt = plt1 + plt2
	d = draw(plt,
		 legend =(
			 position=:top, 
...
function test_and_plot(r, f, A, b, restart_length, exact)
	...
	draw(plt, 
		 ...
		 figure = (;
			 title = "Heat Equation Error for Restart Length $(restart_length)",
			 size=(650,650)
		 )
		)
...
begin
	...
	abs_err_ra = norm(exact - approx_ra[1])
	abs_err_full = norm(exact - approx_full[1])
